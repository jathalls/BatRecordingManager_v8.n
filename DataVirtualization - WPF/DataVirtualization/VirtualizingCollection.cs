...
namespace DataVirtualization
{
    ...
    public class VirtualizingCollection<T> : IList<DataWrapper<T>>, IList where T : class
    {
        ...
        object IList.this[int index]
        {
            get
            {
                DataWrapper<T> dw = this[index];
                if (dw.Data == null) return (DefaultValue);
                else return (dw.Data);
            }
            set { throw new NotSupportedException(); }
        }
        ...
        public void CleanUpPages()
        {
            ...
            {
                //Trace.WriteLine($"time since touch for page {key} is {(DateTime.Now - _pages[key].TouchTime).TotalMilliseconds} :: {PageTimeout}");
